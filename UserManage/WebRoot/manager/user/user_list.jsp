...
<html>
	<head>
		...
		<script type="text/javascript">
			function gopage(p){
				var url="<%=basePath%>/manager/user!list.action?page.cpage="+p
				alert(url);
				window.open(url,"_self");
			}
			function deleted(id){
				$.getJSON("<%=basePath%>/manager/user!delete.action",{'user.id':id},function(msg){
					alert(msg.msg);
					window.location.reload();
				});
			}
			function shouquan(id,d){
				$.getJSON("<%=basePath%>/manager/user!shouquan.action",{'user.id':id,'user.state':d},function(msg){
					alert(msg.msg);
					window.location.reload();
				});
			}
	...
	<body>
		...
		<table width="99%" cellspacing="0" cellpadding="2" border="0"
			...
			<tbody>
				<tr>
					<th>用户名</th><th>真实姓名</th><th>手机号</th><th>邮箱</th><th>IMEI</th><th>访问时间</th><th>状态</th><th>操作&nbsp;<a href="manager/user/user_add.jsp" target="main">添加浏览用户</a></th>
				</tr>
