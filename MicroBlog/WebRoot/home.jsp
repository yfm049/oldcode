...
<form id="" name="" method="post" action="servlet/SearchServlet">
       ...
       <!-- weibo 开始-->
        ...
        // out.print(u_id123+"--------------------------------");
		  ...
		  ResultSet rs = dbConn.execQuery(sql, new Object[]{ u_id123,u_id123,u_id123 });
          ...
          %>
				<tr>
					<td rowspan="3" align="center" valign="top">
						<img src="<%=rs.getString("w_upic")%>" width="50" height="50" />
						</td>
						<td width="88%" class="content">
								<a href="user.jsp"><%=rs.getString("w_unick")%></a>
								<img src="icon/v.gif" width="11" height="10" align="middle" />
								： <%=rs.getString("w_content")%>
							</td>
						</tr>
						<tr>
							<td ><%if(rs.getString("w_image")!= null) {%>
							
								<img src="<%=rs.getString("w_image")%>" width="120" height="120" />
								<%} 
								%>
							</td>
						</tr>
							...
							<td height="25">
								<table width="100%" border="0" cellpadding="0" cellspacing="0"
									...
									<tr>
										...
										<td align="right">
										<a href="servlet/ZhuanfaServlet?id=<%=rs.getInt("id")%>&zhuanw_id=${sessionScope.user.u_id}&zhuanw_unick=${sessionScope.user.u_nickname}&zhuanw_upic=${sessionScope.user.u_pic}&zhuanw_image=<%=rs.getString("w_image")%>&zhuanw_content=<%=(rs.getString("w_content"))%>" style="text-decoration: none">	转发</a>(<%=rs.getString("w_times")%>)&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
										<a href="servlet/CollectionServlet?zhuanw_id=${sessionScope.user.u_id}&zhuanw_unick=${sessionScope.user.u_nickname}&zhuanw_upic=${sessionScope.user.u_pic}&zhuanw_image=<%=rs.getString("w_image")%>&zhuanw_content=<%=(rs.getString("w_content"))%>" style="text-decoration: none">收藏</a>&nbsp;&nbsp;&nbsp;&nbsp;|</a>
										<a href="servlet/ExportServlet?w_content=<%=rs.getString("w_content")%>" style="text-decoration: none">查看当前信息</a>
										</td>
									</tr>
									<tr>
    ...
    <table align="center" id="page">
      <tr>
        <td align="right">第<%=intPage %>页        共<%=intPageCount%>页 
        <% if(intPage>1){%>
          &nbsp; &nbsp; <a href ="home.jsp?page=<%=intPage-1 %>">上一页</a>
      <%
         }
       if(intPage<intPageCount){%>
        <a href ="home.jsp?page=<%=intPage+1 %>">下一页</a></td>
       <%
       }
      rs.close();
      dbConn.closeConn();
    ...
    <!-- weibo 结束-->
  	...
  	<table border="0" align="center" cellpadding="0" cellspacing="0" id="userlist">
        ...
        // out.print(u_id12+"--------------------------------");
          ...
          ResultSet rs1 = dbConn.execQuery(sql1, new Object[]{u_id12,u_id12});
        ...
        <div>
         <%if(intPage1<intPageCount1){ %>
            <a href ="home.jsp?page=<%=intPage%>" class="huanyihuan " style="text-decoration: none"><font color="black"> 换一换</font></a>
             <%
          }
         rs1.close();
